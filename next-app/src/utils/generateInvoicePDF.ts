...
export const generateInvoicePDF = async (
  ...
  const rawTax = typeof order.tax === "number" ? order.tax : parseFloat((order.tax as any) || "0") || 0;
  ...
  itemsList.forEach((item: any) => {
    ...
    const itemRateRaw =
      item.taxRate ??
      item.tax_rate ??
      (item.cgstRate != null && item.sgstRate != null ? (parseFloat(String(item.cgstRate)) + parseFloat(String(item.sgstRate))) : null) ??
      item.igstRate ??
      item.product?.igst ??
      (item.product?.cgst != null && item.product?.sgst != null ? (parseFloat(String(item.product.cgst)) + parseFloat(String(item.product.sgst))) : null) ??
    ...
    const itemRate = !isNaN(parsedRate) && parsedRate >= 0 ? parsedRate : 0;
    ...
    const itemTotalCalculated = typeof item.total === "number"
      ? item.total
      : (item.total ? parseFloat(item.total) : (taxableValNum + itemTaxAmount));
    ...
    taxInfoParts.push(`GST: ${itemRate.toFixed(1)}%`);
    ...
    doc.text(itemTotalCalculated.toFixed(2), 196, currentY, { align: "right" });
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(15, 23, 42);
  doc.text("Taxable Value:", 140, currentY);
  doc.text(`Rs. ${rawSubtotal.toFixed(2)}`, 196, currentY, { align: "right" });

  if (rawTax > 0) {
    currentY += 5;
    doc.text("Total GST / Tax:", 140, currentY);
    doc.text(`Rs. ${rawTax.toFixed(2)}`, 196, currentY, { align: "right" });
  }

  if (rawShipping > 0) {
    currentY += 5;
    doc.text("Shipping Fee:", 140, currentY);
