...
export class OrdersService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      const unassigned = await this.db.select().from(orders).where(
        and(
          isNull(orders.invoiceNumber),
          or(eq(orders.paymentMethod, 'cash_on_delivery'), eq(orders.paymentStatus, 'paid')),
        ),
  ...
  private async findUserOrderByIdOrNumber(userId: number, idOrNumber: string | number) {
    ...
    if (rawOrder.paymentMethod === 'online' && rawOrder.paymentStatus !== 'paid') return null;
  ...
  async placeOrderFromCart(userId: number, body: any): Promise<any> {
    ...
    // Validate COD eligibility across all items
    for (const cartItem of validatedCartItems) {
      const isCodAllowed = cartItem.freshVariant?.isCodAllowed !== false;
      if (!isCodAllowed) {
        throw new BadRequestException(
          `Cash on Delivery is unavailable for item "${cartItem.freshProduct?.name || 'Product'}". Please select Online Payment.`
        );
      }
    }
    ...
    for (const cartItem of validatedCartItems) {
      ...
      const itemTaxable = unitPrice * cartItem.quantity;
      const itemTax = calculateItemTax(cartItem.freshProduct, itemTaxable, sanitizedAddress, true);
      const variantShipFee = parseFloat(String(cartItem.freshVariant?.shippingCharges || '0')) || 0;

      subtotalTaxable += itemTaxable;
      totalTax += parseFloat(itemTax.taxAmount);
      totalShippingFee += variantShipFee * cartItem.quantity;
    ...
    for (const { cartItem, unitPrice, itemTax, variantShipFee } of itemsWithTax) {
      await this.db.insert(orderItems).values({
        ...
        isCodAllowed: cartItem.freshVariant?.isCodAllowed !== false,
        isReturnable: cartItem.freshVariant?.isReturnable !== false,
        returnWindowDays: Number(cartItem.freshVariant?.returnWindowDays ?? 7),
    ...
    const cartIdsToDelete = validatedCartItems.map((c: any) => c.id).filter(Boolean);
    if (cartIdsToDelete.length) {
      await this.db.delete(carts).where(inArray(carts.id, cartIdsToDelete));
    }

    // Fetch the inserted order with its relations
    const orderRows = await this.db.select().from(orders).where(eq(orders.id, r.id)).limit(1);
    const fullOrder = await this.populateSingleOrder(orderRows[0]);

    // Send Order Placed Email (Pending Admin Confirmation)
    this.sendOrderPlacedEmail(r.id).catch(err => this.logger.error('Failed to send order placed email:', err));

    const orderSlug = getOrderSlug(fullOrder) || fullOrder?.orderNumber || r.id;
    ...
    this.notificationsService.createAndEmitNotification({
      ...
      message: `Your order #${fullOrder?.orderNumber || r.id} for ₹${(fullOrder as any)?.total || total.toFixed(2)} has been received!`,
      type: 'ORDER_PLACED',
      priority: 'HIGH',
      entityType: 'order',
      entityId: r.id,
      ...
      metadata: { orderId: r.id, orderNumber: fullOrder?.orderNumber, totalAmount: total.toFixed(2) }
    ...
    this.notificationsService.createAndEmitNotification({
      ...
      message: `New order #${fullOrder?.orderNumber || r.id} placed for ₹${(fullOrder as any)?.total || total.toFixed(2)}`,
      type: 'ORDER_PLACED',
      priority: 'HIGH',
      entityType: 'order',
      entityId: r.id,
  ...
  async createDirectOrder(userId: number, user: any, body: any) {
    ...
    if (!product || product.status === false) {
      throw new BadRequestException('Sorry! The selected product is currently unavailable.');
    }

    const variant = await this.db.query.variants.findFirst({
      where: eq(variants.id, body.variant_id),
    });

    if (!variant || variant.status === false || variant.deletedAt !== null) {
      throw new BadRequestException('Sorry! The selected variant is currently unavailable.');
    }
    ...
    if (variant.isCodAllowed === false) {
      throw new BadRequestException('Cash on Delivery is unavailable for this product variant. Please select Online Payment.');
    }
    ...
    const unitBasePrice = parseFloat(variant.sp as string);
    const itemTaxable = unitBasePrice * body.quantity;
    ...
    const subtotal = itemTaxable;
    const tax = parseFloat(itemTax.taxAmount);
    const variantShipFee = (parseFloat(String(variant.shippingCharges || '0')) || 0) * body.quantity;
    const total = subtotal + tax + variantShipFee;
    const orderNumber = this.generateOrderNumber();
    const invoiceNumber = await this.generateUniqueInvoiceNumber();
    ...
    const [r] = await this.db.insert(orders).values({
      ...
      shippingFee: variantShipFee.toFixed(2),
      tax: tax.toFixed(2),
      total: total.toFixed(2),
      shippingAddress: sanitizedAddress,
      billingAddress: body.billing_address ? validateAndSanitizeShippingAddress(body.billing_address).sanitizedAddress : sanitizedAddress,
    ...
    await this.db.insert(orderItems).values({
      ...
      igstRate: itemTax.igstRate,
      igstAmount: itemTax.igstAmount,
      isCodAllowed: (variant as any).isCodAllowed !== false,
      isReturnable: (variant as any).isReturnable !== false,
      returnWindowDays: Number((variant as any).returnWindowDays ?? 7),
      shippingCharge: (parseFloat(String((variant as any).shippingCharges || '0')) || 0).toFixed(2),
    });
  ...
  // ─── User: place single item direct order ─────────────────────────────────
  async placeSingleItemOrder(userId: number, body: any): Promise<any> {
    return this.createDirectOrder(userId, null, body);
  }
  ...
  async publicTrackOrder(identifier?: string, email?: string): Promise<any> {
    ...
    if (foundOrder.paymentMethod === 'online' && foundOrder.paymentStatus !== 'paid') {
      throw new NotFoundException(`No placed order found matching Order ID / Number "${cleanIdentifier}". Please check your order details.`);
    }
  ...
  async adminUpdateStatus(idOrNumber: string | number, body: any): Promise<any> {
    ...
    if (newStatus === 'confirmed' && order.paymentMethod === 'online' && order.paymentStatus !== 'paid') {
      throw new BadRequestException(
        `Cannot confirm Order #${order.orderNumber}: Online payment has not been received (Payment Status: "${order.paymentStatus}"). Only paid online orders can be confirmed.`
      );
    }
  ...
  async bulkAcceptOrders(body: { orderIds: number[] }): Promise<any> {
    ...
    for (const id of uniqueOrderIds) {
      try {
        ...
        // Safeguard: Prevent bulk confirming unpaid online orders
        if (order.paymentMethod === 'online' && order.paymentStatus !== 'paid') {
          ...
          results.push({
            ...
            status: 'skipped',
            success: false,
            reason: 'Unpaid online order',
            message: `Cannot accept Order #${orderNum} because online payment was not completed (Payment Status: ${order.paymentStatus})`,
          });
          continue;
        }
  ...
  async getOrderStats(): Promise<any> {
    const all = await this.db.select().from(orders).where(
      or(eq(orders.paymentMethod, 'cash_on_delivery'), eq(orders.paymentStatus, 'paid')),
    );
    const now = new Date();
    const stats = {
  ...
  async getInvoicePdfBuffer(orderIdOrNumber: number | string, userId?: number): Promise<{ filename: string; buffer: Buffer }> {
    ...
    if (order.paymentMethod === 'online' && order.paymentStatus !== 'paid') {
      throw new BadRequestException(`Cannot generate tax invoice for unpaid online order #${order.orderNumber}.`);
    }
  ...
  private async buildFilteredGSTRows(query: any): Promise<any[]> {
    ...
    // Exclude unpaid online payment sessions
    allOrders = allOrders.filter(o => o.paymentMethod === 'cash_on_delivery' || o.paymentStatus === 'paid');
