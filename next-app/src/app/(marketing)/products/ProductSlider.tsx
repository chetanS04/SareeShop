...
import { fetchBestsellerProducts } from "../../../../utils/product";
...
const imageUrl = `${process.env.NEXT_PUBLIC_UPLOAD_BASE || "https://api.zelton.co.in"}`;
...
export default function ProductSlider() {
    ...
    const parseProductItem = (prod: any): Product => {
        const prodImg = prod.image_url
    ...
    const loadProducts = async (pageNum: number = 1, isAppend: boolean = false) => {
        ...
        try {
            const res = await fetchBestsellerProducts({ page: pageNum, limit: 10 });
            let list: any[] = [];
            let hasNext = false;
            ...
            const parsedList = list.map(parseProductItem);
    ...
    // Prepare repeated categories for infinite loop scrolling
    ...
    // Continuous auto-sliding animation in an infinite loop
    useEffect(() => {
    ...
    // Horizontal scroll and mouse drag handlers for products
    ...
    const onMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // Check if user dragged near end to fetch next page
                    ...
                    <div className="flex gap-4 sm:gap-5 items-stretch relative group/slider px-1 sm:px-2">
                        ...
                        <div className="relative flex-1 min-w-0">
                            ...
                            >
                                ...
                                {/* Inline loading skeleton cards while sliding to fetch next 10 products */}
                                {isLoadingMore && (
                                    Array.from({ length: 3 }).map((_, idx) => (
                                        <div key={`bs-skeleton-${idx}`} className="flex-shrink-0 w-[175px] sm:w-[210px] md:w-[240px] lg:w-[260px]">
                ...
                >
                    <div className="flex items-center justify-between gap-2 sm:gap-3">
                        ...
                        <button
                            ...
                            className="flex-shrink-0 w-8 h-8 rounded-full border border-gray-300 hover:border-black flex items-center justify-center text-gray-700 hover:text-black transition-colors bg-white shadow-xs hover:shadow-sm cursor-pointer"
