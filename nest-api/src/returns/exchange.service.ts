...
export class ExchangeService {
  ...
  async processReplacementOrder(returnRequestId: number, adminId?: number | null): Promise<any> {
    ...
    for (const item of items) {
      ...
      const itemTaxable = itemPrice * item.quantity;
      const itemTax = calculateItemTax(item.product, itemTaxable, shippingAddressStr, true);
      ...
      replacementItemsData.push({
        ...
        isCodAllowed: (targetVar as any)?.isCodAllowed !== false,
        isReturnable: (targetVar as any)?.isReturnable !== false,
        returnWindowDays: Number((targetVar as any)?.returnWindowDays ?? 7),
        shippingCharge: '0.00',
      });
    }
    ...
    for (const rItem of replacementItemsData) {
      await this.db.insert(orderItems).values({
        ...
        isCodAllowed: rItem.isCodAllowed,
        isReturnable: rItem.isReturnable,
        returnWindowDays: rItem.returnWindowDays,
        shippingCharge: rItem.shippingCharge,
        createdAt: new Date(),
      });
    }
