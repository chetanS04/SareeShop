...
export const databaseProvider: Provider = {
  ...
  useFactory: async (configService: ConfigService) => {
    ...
    try {
      ...
      // Sync products tax columns (hsn, cgst, sgst, igst)
      try {
        const [pCols]: any = await connection.query(`SHOW COLUMNS FROM \`products\``);
        const pColNames = (pCols || []).map((c: any) => c.Field);
        if (!pColNames.includes('hsn')) {
          await connection.query(`ALTER TABLE \`products\` ADD COLUMN \`hsn\` varchar(15) NULL`);
        }
        if (!pColNames.includes('cgst')) {
          await connection.query(`ALTER TABLE \`products\` ADD COLUMN \`cgst\` decimal(5,2) NULL`);
        }
        if (!pColNames.includes('sgst')) {
          await connection.query(`ALTER TABLE \`products\` ADD COLUMN \`sgst\` decimal(5,2) NULL`);
        }
        if (!pColNames.includes('igst')) {
          await connection.query(`ALTER TABLE \`products\` ADD COLUMN \`igst\` decimal(5,2) NULL`);
        }
      } catch (pErr: any) {
        logger.warn(`Products tax column sync note: ${pErr.message}`);
      }

      // Sync variants policy columns
      try {
        const [vCols]: any = await connection.query(`SHOW COLUMNS FROM \`variants\``);
        const vColNames = (vCols || []).map((c: any) => c.Field);
        if (!vColNames.includes('is_cod_allowed')) {
          await connection.query(`ALTER TABLE \`variants\` ADD COLUMN \`is_cod_allowed\` tinyint(1) NOT NULL DEFAULT 1`);
        }
        if (!vColNames.includes('is_returnable')) {
          await connection.query(`ALTER TABLE \`variants\` ADD COLUMN \`is_returnable\` tinyint(1) NOT NULL DEFAULT 1`);
        }
        if (!vColNames.includes('return_window_days')) {
          await connection.query(`ALTER TABLE \`variants\` ADD COLUMN \`return_window_days\` int NOT NULL DEFAULT 7`);
        }
        if (!vColNames.includes('shipping_charges')) {
          await connection.query(`ALTER TABLE \`variants\` ADD COLUMN \`shipping_charges\` decimal(10,2) NOT NULL DEFAULT 0.00`);
        }
      } catch (vErr: any) {
        logger.warn(`Variants policy column sync note: ${vErr.message}`);
      }

      // Sync order_items snapshot & tax columns
      try {
        const [oiCols]: any = await connection.query(`SHOW COLUMNS FROM \`order_items\``);
        const oiColNames = (oiCols || []).map((c: any) => c.Field);
        if (!oiColNames.includes('is_cod_allowed')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`is_cod_allowed\` tinyint(1) NULL DEFAULT 1`);
        }
        if (!oiColNames.includes('is_returnable')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`is_returnable\` tinyint(1) NULL DEFAULT 1`);
        }
        if (!oiColNames.includes('return_window_days')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`return_window_days\` int NULL DEFAULT 7`);
        }
        if (!oiColNames.includes('shipping_charge')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`shipping_charge\` decimal(10,2) NULL DEFAULT 0.00`);
        }
        if (!oiColNames.includes('hsn')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`hsn\` varchar(15) NULL`);
        }
        if (!oiColNames.includes('tax_rate')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`tax_rate\` decimal(5,2) NULL`);
        }
        if (!oiColNames.includes('taxable_amount')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`taxable_amount\` decimal(10,2) NULL`);
        }
        if (!oiColNames.includes('tax_amount')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`tax_amount\` decimal(10,2) NULL`);
        }
        if (!oiColNames.includes('cgst_rate')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`cgst_rate\` decimal(5,2) NULL`);
        }
        if (!oiColNames.includes('cgst_amount')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`cgst_amount\` decimal(10,2) NULL`);
        }
        if (!oiColNames.includes('sgst_rate')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`sgst_rate\` decimal(5,2) NULL`);
        }
        if (!oiColNames.includes('sgst_amount')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`sgst_amount\` decimal(10,2) NULL`);
        }
        if (!oiColNames.includes('igst_rate')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`igst_rate\` decimal(5,2) NULL`);
        }
        if (!oiColNames.includes('igst_amount')) {
          await connection.query(`ALTER TABLE \`order_items\` ADD COLUMN \`igst_amount\` decimal(10,2) NULL`);
        }
      } catch (oiErr: any) {
        logger.warn(`Order items policy/tax column sync note: ${oiErr.message}`);
      }

      // Cleanup deprecated tax columns from categories if they still exist
      try {
        const [cCols]: any = await connection.query(`SHOW COLUMNS FROM \`categories\``);
        const cColNames = (cCols || []).map((c: any) => c.Field);
        for (const col of ['hsn', 'cgst', 'sgst', 'igst']) {
          if (cColNames.includes(col)) {
            await connection.query(`ALTER TABLE \`categories\` DROP COLUMN \`${col}\``);
          }
        }
      } catch (cErr: any) {
        logger.warn(`Categories tax column cleanup note: ${cErr.message}`);
      }
