                                        ...
                                        <div className={`flex gap-4 py-5 rounded-lg transition-colors ${stockInfo.isOutOfStock ? 'bg-red-50/40 p-3 -mx-3 border border-red-100' : ''}`}>
                                            ...
                                            <div className="flex-1 min-w-0 flex flex-col sm:flex-row gap-3">
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    {/* Variant Policy Badges */}
                                                    <div className="flex flex-wrap items-center gap-1.5 mb-2.5 mt-1">
                                                        {/* Return policy badge */}
                                                        ...
                                                        {(Number((item.variant as any)?.shipping_charges ?? (item.variant as any)?.shippingCharges ?? 0) > 0) ? (
                                                            <span className="text-[11px] font-medium text-slate-700 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded">
                                                                + ₹{Number((item.variant as any)?.shipping_charges ?? (item.variant as any)?.shippingCharges) * item.quantity} Shipping
                                                            </span>
                                                        ) : (
                                                        ...
                                                        {/* COD badge if disabled */}
                                                        {(((item.variant as any)?.is_cod_allowed ?? (item.variant as any)?.isCodAllowed) === false || ((item.variant as any)?.is_cod_allowed ?? (item.variant as any)?.isCodAllowed) === 0) && (
                            ...
                            <div className="flex justify-between text-sm text-gray-600 mb-4">
                                ...
                                {(() => {
                                    const totalShipping = items.reduce((sum, item) => {
                                        ...
                                        return sum + (parseFloat(String(rawShipping)) || 0) * (item.quantity || 1);
                                    }, 0);
                                    return totalShipping > 0 ? (
                                        <span className="font-semibold text-gray-900">₹{totalShipping}</span>
                                    ) : (
                            ...
                            <div className="flex justify-between font-bold text-gray-900 text-base mb-4">
                                ...
                                <span>
                                    ₹{(() => {
                                        ...
                                        return Math.round((Number(total) + totalShipping + Number.EPSILON) * 100) / 100;
                                    })()}
                                </span>
                            </div>
