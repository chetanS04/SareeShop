...
export class DelhiveryService {
  ...
  private async createMockShipment(orderId: number, order: any): Promise<any> {
    if (order.paymentMethod === 'online' && order.paymentStatus !== 'paid') {
      throw new BadRequestException(
        `Cannot create shipment for Order #${order.orderNumber}: Online payment has not been received (Payment Status: ${order.paymentStatus}).`
      );
    }
  ...
  async createShipment(orderId: number | string, body: any = {}): Promise<any> {
    ...
    // Safeguard: Prevent creating shipment for unpaid online orders
    if (order.paymentMethod === 'online' && order.paymentStatus !== 'paid') {
      throw new BadRequestException(
        `Cannot create shipment for Order #${order.orderNumber}: Online payment has not been received (Payment Status: "${order.paymentStatus}"). Only paid orders can be shipped.`
      );
    }
  ...
  async bulkCreateShipments(body: { orderIds: number[] }): Promise<any> {
    ...
    for (const id of uniqueOrderIds) {
      try {
        ...
        // Validation 4: Unpaid online order
        if (order.paymentMethod === 'online' && order.paymentStatus !== 'paid') {
          skippedCount++;
          results.push({
            orderId: id,
            ...
            message: `Cannot create shipment for #${orderNum} because online payment was not completed`,
          });
          continue;
        }
