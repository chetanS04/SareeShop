      ...
      <div className="max-w-site mx-auto site-pad section-y">
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-on-surface/15 pb-5 sm:pb-6 mb-6 sm:mb-8 gap-4">
          <div>
            ...
            <h2 id="edit-title" className="display-section text-on-surface uppercase">
              The Independent Cut
            </h2>
            <p className="text-[14px] sm:text-[15px] text-body-slate mt-2 max-w-xl">
              Architectural silhouettes from the current archive — pieces that refuse seasonal noise.
            </p>
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="aspect-[3/4] bg-surface-ivory border border-[rgba(14,14,13,0.08)] animate-pulse"
              />
            ))}
          </div>
        ) : items.length > 0 ? (
