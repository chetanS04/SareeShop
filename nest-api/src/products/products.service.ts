...
export class ProductsService {
  ...
  async getProductById(idOrSlug: string | number, req?: any): Promise<any> {
    ...
    const formattedVariants = variantsToFormat.map((v: any) => {
      const attrVals = (v.variantAttributeValues ?? []).map((vav: any) => ({
        ...
        image: vav.attributeValue?.image ?? null,
        show_image: vav.attributeValue?.showImage !== undefined ? Boolean(vav.attributeValue?.showImage) : (vav.attributeValue?.show_image !== undefined ? Boolean(vav.attributeValue?.show_image) : false),
        showImage: vav.attributeValue?.showImage !== undefined ? Boolean(vav.attributeValue?.showImage) : (vav.attributeValue?.show_image !== undefined ? Boolean(vav.attributeValue?.show_image) : false),
  ...
  async getBestsellerProducts(query: any = {}): Promise<any> {
    ...
    const mapped = (activeProducts as any[]).map((product: any) => {
      ...
      const bestVariant = sortedVariants[0] ? {
        ...
        stock: sortedVariants[0].stock,
        image_url: sortedVariants[0].imageUrl,
        bs: (sortedVariants[0] as any).bs ?? 0,
      } : null;
  ...
  public async findFullProductById(id: number, isAdmin: boolean = false): Promise<any> {
    ...
    const cgstNum = parseFloat(String(cgst ?? '0')) || 0;
    const sgstNum = parseFloat(String(sgst ?? '0')) || 0;
    const gstRate: number = igstNum > 0 ? igstNum : (cgstNum + sgstNum > 0 ? cgstNum + sgstNum : 18.0);
    ...
    if (varIds.length > 0) {
      ...
      for (const row of vavRows) {
        ...
        vavByVariant.get(vid)!.push({
          ...
          attributeValue: {
            ...row.attributeValue,
            image: row.attributeValue?.image ?? null,
            show_image: Boolean(row.attributeValue?.showImage),
            showImage: Boolean(row.attributeValue?.showImage),
