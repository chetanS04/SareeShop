...
export default function ReturnRequestModal({
  ...
  >(() => {
    ...
    const firstEligible = eligibleItems.find(
      (it: any) => it.isEligible !== false && it.isReturnable !== false && (it.availableReturnQuantity || it.returnable_quantity || 0) > 0
    );
    if (firstEligible) {
      initial[firstEligible.orderItemId] = {
        quantity: 1,
        exchangeVariantId: firstEligible.exchangeVariants?.[0]?.variantId,
      };
    }
    return initial;
  });
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Choose items from your order that are eligible for return or exchange under variant policy.
                </p>
              </div>

              <div className="space-y-3">
                {eligibleItems.map((item: any) => {
                  const isItemEligible = item.isEligible !== false && (item.isReturnable ?? true) && (item.availableReturnQuantity || item.returnable_quantity || 0) > 0;
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                        !isItemEligible
                          ? "border-gray-200 bg-slate-50/80 opacity-70 cursor-not-allowed"
                          : isSelected
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="w-5 h-5 rounded text-[#007FFF] focus:ring-blue-500 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                        />
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-0.5">
                            ...
                            {item.isReturnable === false ? (
                              ...
                              <span className="text-[10px] font-medium text-blue-700 bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded">
                                {item.remainingDays} days window
                              </span>
                            ) : null}
                          </div>
                          ...
                          {!isItemEligible ? (
                            <p className="text-[11px] font-semibold text-rose-600 mt-1">
                              ⚠️ {item.ineligibilityReason || "Not eligible for return/exchange"}
                            </p>
                          ) : (
