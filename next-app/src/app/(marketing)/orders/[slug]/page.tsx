                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-1.5">
                                ...
                                {item.variant.sku && (
                                 ...
                                 {(() => {
                                  const isRet =
                                    item.isReturnable !== undefined && item.isReturnable !== null
                                      ? Boolean(item.isReturnable)
                                      : item.is_returnable !== undefined && item.is_returnable !== null
                                      ? Boolean(item.is_returnable)
                                      : (item.variant?.isReturnable ?? item.variant?.is_returnable ?? true);
                                  const windowDays =
                                    item.returnWindowDays ?? item.return_window_days ?? item.variant?.returnWindowDays ?? item.variant?.return_window_days ?? 7;

                                  return !isRet ? (
                                    <span className="px-2 py-0.5 bg-rose-50 border border-rose-200 text-rose-700 text-xs font-semibold rounded">
                                      Non-Returnable
                                    </span>
                                  ) : (
                                    <span className="px-2 py-0.5 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-semibold rounded">
                                      {windowDays} Days Return Window
                                    </span>
                                  );
                                })()}
                                {Number(item.shippingCharge ?? item.shipping_charge ?? item.variant?.shippingCharges ?? item.variant?.shipping_charges ?? 0) > 0 && (
                                  <span className="px-2 py-0.5 bg-slate-100 text-slate-700 text-xs font-semibold rounded">
                                    + ₹{Number(item.shippingCharge ?? item.shipping_charge ?? item.variant?.shippingCharges ?? item.variant?.shipping_charges)} Shipping
                                  </span>
                                )}
                              </div>
                            </div>
