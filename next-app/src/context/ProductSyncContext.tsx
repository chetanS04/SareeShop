...
interface ProductSyncContextType {
  socket: Socket | null;
  isConnected: boolean;
  subscribeToProduct: (productId: number | string, callback: (event: ProductEventData) => void) => () => void;
  subscribeToAll: (callback: (event: ProductEventData) => void) => () => void;
  joinProductRoom: (productId: number | string) => void;
  leaveProductRoom: (productId: number | string) => void;
}
...
export const ProductSyncProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const productListenersRef = useRef<Map<string | number, Set<(event: ProductEventData) => void>>>(new Map());
  ...
  useEffect(() => {
    ...
    const dispatchEvent = (eventData: ProductEventData) => {
      ...
      const slug = eventData.product?.slug;

      const targetKeys = new Set<string | number>();
      if (pid !== undefined) {
        targetKeys.add(pid);
        targetKeys.add(Number(pid));
        targetKeys.add(String(pid));
      }
      if (slug) {
        targetKeys.add(slug);
        targetKeys.add(String(slug).toLowerCase().trim());
      }

      const calledCallbacks = new Set<(event: ProductEventData) => void>();
      targetKeys.forEach((key) => {
        const set = productListenersRef.current.get(key);
        if (set) {
          set.forEach((cb) => {
            if (!calledCallbacks.has(cb)) {
              ...
              try {
                ...
                console.error(`Error in product (${key}) socket listener:`, e);
              }
            }
          });
        }
  ...
  const subscribeToProduct = useCallback(
    (productId: number | string, callback: (event: ProductEventData) => void) => {
      const key = typeof productId === "string" && !isNaN(Number(productId)) && productId.trim() !== "" 
        ? Number(productId) 
        : productId;

      if (!productListenersRef.current.has(key)) {
        productListenersRef.current.set(key, new Set());
      }
      productListenersRef.current.get(key)!.add(callback);
      ...
      return () => {
        ...
        if (set) {
          set.delete(callback);
          if (set.size === 0) {
            productListenersRef.current.delete(key);
