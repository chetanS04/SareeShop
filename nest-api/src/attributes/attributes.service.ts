...
export class AttributesService {
  ...
  async storeValue(body: any): Promise<any> {
    ...
    for (const item of valueList) {
      ...
      const itemImage = item.image !== undefined ? (item.image || null) : (body.image || null);
      const itemShowImage = item.show_image !== undefined
        ? Boolean(item.show_image)
        : (item.showImage !== undefined
          ? Boolean(item.showImage)
          : (body.show_image !== undefined
            ? Boolean(body.show_image)
            : (body.showImage !== undefined ? Boolean(body.showImage) : false)));
  ...
  async updateValue(id: number, body: any): Promise<any> {
    ...
    const updatedImage = body.image !== undefined ? (body.image || null) : existingVal.image;
    const updatedShowImage = body.show_image !== undefined
      ? Boolean(body.show_image)
      : (body.showImage !== undefined ? Boolean(body.showImage) : existingVal.showImage);
  ...
  async changeValueShowImage(id: number): Promise<any> {
    const val = await this.db.query.attributeValues.findFirst({
      where: eq(attributeValues.id, id),
    });
    if (!val) {
    ...
    const newShowImage = !val.showImage;
    await this.db
      .update(attributeValues)
      .set({ showImage: newShowImage })
      .where(eq(attributeValues.id, id));

    return {
      result: 'success',
      message: 'Show image status updated successfully',
      show_image: newShowImage,
  ...
  private formatAttributeValue(val: any) {
    ...
    return {
      ...
      show_image: Boolean(val.showImage ?? val.show_image),
      showImage: Boolean(val.showImage ?? val.show_image),
