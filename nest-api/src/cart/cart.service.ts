...
export class CartService {
  ...
  async index(userId: number) {
    try {
      ...
      const items = rawItems.map((item: any) => {
        ...
        const igstNum = parseFloat(String(p?.igst ?? '0')) || 0;
        const cgstNum = parseFloat(String(p?.cgst ?? '0')) || 0;
        const sgstNum = parseFloat(String(p?.sgst ?? '0')) || 0;
        const gstRate = igstNum > 0 ? igstNum : (cgstNum + sgstNum > 0 ? cgstNum + sgstNum : (parseFloat(String(p?.gstRate ?? 18)) || 18.0));
        ...
        const baseSp =
          v?.sp !== null && v?.sp !== undefined
            ...
            : p?.sp !== null && p?.sp !== undefined
              ? Number(p.sp)
              : 0;
        const rawGst = baseSp * (gstRate / 100);
        const displayPrice = Math.round(baseSp + rawGst);
        const gstAmount = Math.round(rawGst);
        const totalNum = Math.round(displayPrice * item.quantity);
        ...
        return {
          ...
          variant: v
            ? {
                ...
                is_cod_allowed: v.isCodAllowed !== undefined ? Boolean(v.isCodAllowed) : true,
                is_returnable: v.isReturnable !== undefined ? Boolean(v.isReturnable) : true,
                return_window_days: v.returnWindowDays !== null && v.returnWindowDays !== undefined ? Number(v.returnWindowDays) : 7,
                shipping_charges: v.shippingCharges !== null && v.shippingCharges !== undefined ? Number(v.shippingCharges) : 0,
                isCodAllowed: v.isCodAllowed !== undefined ? Boolean(v.isCodAllowed) : true,
                isReturnable: v.isReturnable !== undefined ? Boolean(v.isReturnable) : true,
                returnWindowDays: v.returnWindowDays !== null && v.returnWindowDays !== undefined ? Number(v.returnWindowDays) : 7,
                shippingCharges: v.shippingCharges !== null && v.shippingCharges !== undefined ? Number(v.shippingCharges) : 0,
              }
            : null,
        };
      });
      ...
      if (existingCartItem) {
        ...
        await this.db
          ...
          .set({
            ...
            price: Math.round(itemPrice).toString(),
      ...
      const currentPrice =
        cartItem?.variant?.sp !== null && cartItem?.variant?.sp !== undefined
          ...
          : cartItem?.product?.sp !== null && cartItem?.product?.sp !== undefined
            ...
            : itemPrice;
      const totalNum = Math.round(currentPrice * (cartItem?.quantity || quantity || 1));
  ...
  async update(userId: number, id: number, body: any) {
    try {
      ...
      const totalNum = Math.round(currentPrice * quantity);
